...
**⚠️ 每条 AI 输出首行必须携带任务ID**：
- 格式：`[阶段名称] 任务ID: task_xxxxxxxx`
- 状态更新时携带任务ID
- 禁止脱离任务上下文执行操作

**为什么必须携带**：
- 任务ID通过 AI 输出传递，形成上下文链
- 下一轮对话时，IDE/上下文自动携带上一轮的任务ID
- 防止上下文超限导致任务丢失
...
| 检查项 | 说明 |
| --- | --- |
| 上一轮 AI 输出 | 检查上一轮输出首行 `任务ID: task_xxxxxxxx` |
| IDE 会话上下文 | IDE 自动携带上一轮输出的任务ID |
| 用户输入引用 | 用户引用了包含任务ID的历史输出 |
...
| 阶段 | 自动衔接规则 |
| --- | --- |
| stage5 → stage6 | **强制自动衔接**，禁止等待用户 |
| stage6 → stage7 | **强制自动衔接**，禁止等待用户 |
| stage7 → stage8 | **强制自动衔接**，禁止等待用户 |

**自动衔接执行方式**：
- stage5完成后：立即输出 `[实施研发] 执行完成。进入下一阶段：[内部验证]。` 并执行stage6
- stage6完成后：立即输出 `[内部验证] 执行完成。进入下一阶段：[文档同步]。` 并执行stage7
- stage7完成后：立即输出 `[文档同步] 执行完成。进入下一阶段：[交付续跑]。` 并执行stage8
- **禁止在自动衔接阶段等待用户确认**

**禁止**：未经确认进入下一阶段（显式确认点）、自动衔接阶段等待用户（自动衔接点）、连续输出多阶段内容
