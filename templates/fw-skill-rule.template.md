...
| 携带来源 | 说明 |
| --- | --- |
| 上下文引用 | 用户引用了先前 AI 输出（包含 `任务ID: task_xxxxxxxx`） |
| IDE 自动携带 | IDE 会话管理自动携带上一轮的任务ID |
| 无携带 | 新会话第一条输入，无任何任务ID上下文 |

**判断逻辑**：
| 携带情况 | 后续动作 |
| --- | --- |
| 有任务ID携带 | → 第二步A：读取状态文件 → 找到对应任务 → 继续 |
| **无任务ID携带** | → **第二步B：创建新任务**（新会话第一条输入） |

**重要**：
- 用户只输入需求，不会手动输入任务ID
- 任务ID通过 AI 输出首行携带，后续对话通过引用/上下文自动传递
- 如果检测到"无任务ID携带"，说明是新会话第一条输入，直接创建新任务

### 第二步A：继续已有任务（有任务ID携带）

1. **读取配置文件** `{project_ide_dir}/.fw-session-config.json`
2. **读取状态文件** `{project_ide_dir}/rules/fw-session-state.md`
3. 查找 `<!-- TASK_{任务ID大写}_START -->` → 继续该任务
4. **根据阶段处理输入**：
   | 阶段 | 状态 | 输入类型 | 处理 |
   | --- | --- | --- | --- |
...
3. **新增任务块**（不修改现有活跃任务）
4. **从 Stage 0 开始执行**
5. **首行输出携带任务ID** `[阶段名称] 任务ID: task_{新随机ID}`

### 禁止行为

- **禁止**：在未检查任务ID携带情况前就读取状态文件活跃任务
- **禁止**：假设用户想继续已有任务（必须先检查任务ID是否携带）
- **禁止**：将无关的新请求关联到状态文件中的活跃任务
