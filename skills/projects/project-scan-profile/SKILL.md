---
name: project-scan-profile
description: 扫描前端项目并生成或刷新对应项目技能。用于定位前端根目录、识别项目结构、提取技术栈、UI 框架与研发约束；Stage 1 应先吸收用户提供的项目 skills / 文档与 UI 框架资料，再复用当前工作区已存在的项目专项技能，只有在找不到可用候选时，才按规范生成标准项目技能产物。
---
...
- 主产物：当前目标项目对应的项目技能产物；当前默认产物名固定为 `frontend-workmate-project-skill`
- 按需生成或刷新的项目级 UI 框架 skill
- 若需要新建或标准化归档，则生成标准项目技能产物 `frontend-workmate-project-skill`
- 若需要为自研 UI 框架沉淀技能，则生成对应 UI 技能产物
- `../../../templates/project/project-architecture-profile.md`、`../../../templates/project/project-dev-playbook.md`、`../../../templates/project/project-skill-template.md` 仅作为扫描脚手架
- `../../../templates/project/project-ui-skill-template.md` 仅作为 UI 框架 skill 生成脚手架

## Workflow

0. 对“由用户提供”的输入统一判定状态：`provided`、`not_provided`、`skipped`、`pending`、`not_applicable`；仅 `pending` 阻塞阶段推进
1. 先检查当前工作区已有项目技能候选、目录结构与前端入口，识别前端项目数量与候选目标项目
...
5. 锁定目标项目后，优先按固定技能名检查是否已有可复用项目技能；当前默认优先检查 `frontend-workmate-project-skill`
6. 若已存在项目 skills，先向用户说明“已检测到现有项目 skills，我将检查是否需要更新”；检查完成后，把更新结果交给用户检查，用户若补充内容则继续合并到该 skills
7. 若不存在可复用候选，则本阶段必须先停住，询问用户是否愿意提供现有项目 skills、项目文档或其他可沉淀为项目 skills 的资料；该轮问题属于 `provide`；默认提示语使用：`你可以直接回复提供的内容，我将优先采用您提供的资料生成项目的技能；如果你不提供，我再基于仓库内容继续分析并生成。`
8. 若用户直接提供项目 skills，则按 `../../../templates/project/project-skill-template.md` 归档为标准项目技能产物，并使用固定技能名 `frontend-workmate-project-skill` 作为最高优先级项目技能来源；若已有同名项目 skills，则执行合并/刷新
9. 若用户提供项目文档，则优先基于文档生成或刷新固定项目技能名 `frontend-workmate-project-skill`
10. 若用户明确表示不提供项目 skills / 项目文档（`not_provided`），才按 `../../../SKILL.md` 中定义的发现规则，继续内部扫描并生成首版 `frontend-workmate-project-skill`
11. 只允许把稳定项目知识写入 `frontend-workmate-project-skill`，例如目录结构、技术栈、构建方式、路由模式、UI 框架、长期约束；不得把当前单次任务的临时目标、样式方案、实现偏好、页面级改法写入项目技能产物
...
16. 若用户跳过初始化，则终止流程，不再继续生成项目技能
17. 初始化成功后不得把“项目已创建/依赖已安装”视为 Stage 1 完成；必须继续提取项目结构、入口、命令、底层框架、语言框架、语言、UI 库与约束，并生成首版项目 skills
18. 用 `project-architecture-profile` 模板提取稳定结构、入口、命令、底层框架、语言框架、语言、UI 库与约束
19. 用 `project-dev-playbook` 模板提取路由、权限、API、状态、样式、国际化、构建规则与 UI 库使用约束
20. 若识别到自研 UI 框架/设计系统，且仅已知库名称，则先记录该名称；若未提供版本，默认按最新版处理，不预先追问安装地址、本地路径或私仓来源
...
29. 若当前是用户未提供资料后内部扫描生成的项目 skills，则向用户说明：已基于仓库证据生成该 skills，并请用户检查是否需要补充内容继续完善
30. 若用户认为 Stage 1 产物不准确，或补充了新的项目约束、目录、入口、技术栈信息，必须先判断这些内容是“稳定项目知识”还是“当前单次任务约束”；只有稳定项目知识才允许合并进 `frontend-workmate-project-skill`
31. 合并并刷新后，向用户回显最新项目基线摘要，并根据用户本轮回复类型选择提示语：
   ...
   - 用户明确不提供资料：`我已记录你暂不提供该资料，当前项目 skills 已按仓库证据更新。以上是当前项目基线的最新结果；如果还有偏差或补充，你可以直接告诉我。`
...
- 若仓库内有多个前端子项目，应分别生成或刷新对应项目技能；只有当本次目标项目存在歧义时才向用户询问选哪个
- 项目技能是 Stage 1 的最终产物，模板分析只是生成过程
- 项目技能中应明确记录可复用边界与刷新信号
- 项目技能目录命名应稳定、可预测，并与项目根目录或稳定业务标识一致
