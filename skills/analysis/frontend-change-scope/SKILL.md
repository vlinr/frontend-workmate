---
name: frontend-change-scope
description: Analyze a frontend request after project scanning, determine whether it is a bug, feature, or refactor, and produce change scope plus capability matrix outputs. Use when users ask to analyze impact, define modification scope, decide required skills or MCP, identify missing capabilities, or prepare frontend work before coding.
---
...
- 默认至少更新 `../../../templates/analysis/change-scope.md` 中的阶段字段与范围结论；复杂任务再补充 `../../../templates/analysis/capability-matrix.md`。
...
2. 再扫描当前可用技能来源：用户提供资料、Stage 1 已命中的固定项目技能名、项目内专项 skill、其他可复用公共技能包
3. 优先读取 Stage 1 已命中的固定项目技能名；当前仓库默认优先读取 `frontend-workmate-project-skill`
4. 若目标目录缺少说明文档或实现关系不清晰，调用 `code-analysis-doc`
...
15. 明确后续要触发的技能，例如 `systematic-debugging`、`react-best-practices`、`react-components`
16. 形成当前任务的技能路线，并输出“要做什么 / 为什么这样做 / 暂不做什么 / 还缺什么证据”的清晰分析结论
