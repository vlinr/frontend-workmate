...
- Stage 0 若存在 `scripts/init-skills.js`，应先把【当前技能目录】下的 `skills/curated/`、`skills/external/` 里的技能整理为可直接调用的公共技能包，再进入正式阶段扫描。
...
- 若【工作目录】为空白项目或不存在前端工程，Stage 1 需要先确认是否协助初始化；用户跳过时流程终止。
...
- 修改交付规则或续跑规则时，优先同步 `task-plan-checkpoint` 与 `temp/task-runs/` 的约定。
