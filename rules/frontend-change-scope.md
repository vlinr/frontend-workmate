...
4. 若【代码改动目录】缺少说明文档或实现关系不清晰：
   ...
   - 调用技能 `fw-code-analysis-doc`（优先查找 `{static_config_dir}/skills/`，若未找到则调用 `find-skills` 查找）
...
- `bug` 任务 → 建议规划技能 `fw-systematic-debugging`（Stage 5 实际调用，优先查找 `{static_config_dir}/skills/`）
- React 技术栈（项目技能中标记） → 建议规划技能 `fw-react-best-practices`（仅适用于 React，Stage 5 实际调用）
- React 技术栈且涉及组件开发 → 建议规划技能 `fw-react-components`（仅适用于 React，Stage 5 实际调用）
- 复杂类型问题 → 建议规划技能 `fw-typescript-advanced-types`（Stage 5 实际调用）
- 页面/组件改动 → 建议规划技能 `fw-accessibility`（Stage 6 实际调用）
- 样式/UI 改动 → 建议规划技能 `fw-web-design-guidelines`（Stage 6 实际调用）
